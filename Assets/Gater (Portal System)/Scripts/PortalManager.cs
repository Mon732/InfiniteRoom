	...
	void LateUpdate () {
		if (!PairedPortal || PairedPortal.GetComponent<PortalManager> ().PairedPortal != gameObject) {
			...
			if (!InGameCamera) {
				...
				if (PortalSettings.EnableRender) {
					GetComponent<MeshRenderer> ().enabled = true;
					SceneviewRender.GetComponent<MeshRenderer> ().enabled = true;
				} else {
					GetComponent<MeshRenderer> ().enabled = false;
					SceneviewRender.GetComponent<MeshRenderer> ().enabled = false;
				}
							...
							if (RenderTexts [i]) {
								for (int j = ViewSettings.Recursion.Steps; j >= 0; j--) {
									if (PortalCamObjs [j]) {
										...
										RenderTexture.active = PortalCamObjs [j].GetComponent<Camera> ().targetTexture;
